...
export default function WorkoutView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // FIX: Force IDs to lowercase string to ensure matching
      exData.forEach(ex => exMap[String(ex.id).toLowerCase()] = ex);
      setExercises(exMap);
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="space-y-4 pt-2">
        ...
        {sets.length === 0 ? (
          <p className="text-sm text-zinc-500 italic">No exercises logged.</p>
        ) : (
          <div className="space-y-3">
            {sets.map((set, i) => {
              // FIX: Match with lowercase ID
              const exName = exercises[String(set.exerciseId).toLowerCase()]?.name || "Unknown Exercise";
              return (
